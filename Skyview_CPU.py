# -*- coding: utf-8 -*-
"""
Created on Sat Jul  8 22:49:40 2023
    ...
    for j in prange(nv):
        for i in range(nh):
            ...
            if z>=0 and z<=nz-1 and x>=0 and x<=nx-1 and y>=0 and y<=ny-1:
                ...
                old_zero = True
                old_k_ext = 0.
                old_sca = np.zeros(3,dtype=floatype)
                new_sca = np.zeros(3,dtype=floatype)
                # fast = True
                inside = True
                while inside:
                    # if fast:
...
sca = np.zeros((nz,nx,ny,3),dtype=floatype)
k_ext = np.zeros((nz,nx,ny),dtype=floatype)
